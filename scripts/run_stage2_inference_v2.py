"""
Same as run_stage2_inference.py, but points at the v2 checkpoint (trained with
train_cond_footprint + cond_grid_dilation_kernel, see PROGRESS.md "Fix attempt 2")
and uses model.encode_cond_grid(...) instead of a raw vae._encode(...) call, so
the same dilation amount used in training is automatically applied at test time
too (read from the checkpoint's own saved hparams).

Usage:
    python scripts/run_stage2_inference_v2.py
"""
import sys
...
VERSION_DIR = Path('/home/ameliacatala/Documents/checkpoints/gpr/Diffusion_stage2_v2/version_2')
...
for i in indices:
    ...
    with torch.no_grad():
        # Original: cond_latent = model.vae._encode({DS.INPUT_PC: teunet_grid}, use_mode=True)
