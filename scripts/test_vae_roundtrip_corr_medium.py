"""
Diagnostic: passes Step1's flawed grid (+ material) through the frozen Stage 1
VAE's own encoder + decoder directly (no diffusion, no noise) and compares
the result to ground truth, on the corr_medium dataset.

Mirrors the original TEUNet-era test_vae_roundtrip.py (see PROGRESS.md Part 1),
which found the frozen VAE round-trip alone scores almost identically to the
full trained diffusion model -- meaning the diffusion process wasn't doing
meaningful denoising. That check was never re-run on corr_medium/Step1 data;
this closes that gap before investing more time in diffusion-side training
fixes (v6/v7).

Usage:
    python scripts/test_vae_roundtrip_corr_medium.py
"""
import sys
