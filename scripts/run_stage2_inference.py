"""
Runs the trained Stage 2 diffusion model on real test samples and compares its
output against both the ground truth and TEUNet's original (flawed) input, using
IoU (intersection-over-union of occupied voxels) as the comparison metric.

Usage:
    python scripts/run_stage2_inference.py
"""
import sys
...
# hparams saved inside our own checkpoint. Safe here: always our own checkpoint.
...
# Same custom unpickler as xcube/data/gpr.py, needed to load the .pkl files.
...
# Full test set by default. Pass specific indices as command-line args (0-197) to
# only test a subset, e.g.: python run_stage2_inference.py 0 30 60 90
